...
const SAMPLE_GRID: Grid8 = array_grid!(8, 8; [(1, 1), (2, 1), (2, 2), (4, 4)]);

mod properties {
    use super::*;

    test_self_method!(rect: SAMPLE_GRID.get(RECT_1_1_2_2) => size() => Size::new(2, 2));

    test_self_method!(local_0_0: SAMPLE_GRID.get(RECT_1_1_2_2) => get(Point::new(0, 0)) => Ok(true));
...
mod iter {
    ...
    test_ctor!(
        cells: SAMPLE_GRID.get(RECT_1_1_2_2).cells().collect::<Vec<_>>()
        => [true, true, false, true]
    );
    test_ctor!(
        points: SAMPLE_GRID.get(RECT_1_1_2_2).points().collect::<Vec<_>>()
        => [(0, 0), (1, 0), (1, 1)]
    );
    test_ctor!(
        spaces: SAMPLE_GRID.get(RECT_1_1_2_2).spaces().collect::<Vec<_>>()
        => [(0, 1)]
    );
}
